...
export class CrashRecoveryManager {
  ...
  private crashDetectionJobs: Map<string, Job>;
  ...
  constructor(public readonly options: ICrashRecoveryManagerOptions) {
    ...
    this.crashDetectionJobs = new Map<string, Job>();
  ...
  get className(): string {
    return CrashRecoveryManager.CLASS_NAME + this.type;
  }
    ...
    try {
      ...
      for (const log of allLogs) {
        ...
        try {
          ...
          let TYPE: string;
          if (satpSession.hasClientSessionData()) {
            TYPE = "CLIENT";
          } else if (satpSession.hasServerSessionData()) {
            TYPE = "SERVER";
          } else {
            throw new Error(
              "Session data is missing client or server information.",
            );
          }
          this._TYPE = TYPE;
  ...
  private updateSessionState(sessionId: string, newState: State): SATPSession {
  ...
  private detectCrash() {
    ...
    if (this.sessions.size === 0) {
      this.log.warn(
        `${fnTag} No active sessions. Clearing all scheduled jobs.`,
      );
      this.stopAllCrashDetectionJobs();
      return;
    }
    ...
    for (const [sessionId, session] of this.sessions.entries()) {
      ...
      if (sessionData.state === State.RECOVERED) {
        this.log.info(
          `${fnTag} Session ${sessionId} is already completed. Stopping job.`,
        );
        console.log(1);
        this.stopCrashDetection(sessionId);
        continue;
      }

      if (this.crashDetectionJobs.has(sessionId)) {
        this.log.info(
          `${fnTag} Job already scheduled for session ID: ${sessionId}`,
        );
        continue;
      }
      ...
      const job = schedule.scheduleJob(
        ...
        async () => {
          this.log.debug(
            `${fnTag} Running crash detection for session ID: ${sessionId}`,
          );
          const session = this.sessions.get(sessionId);
          ...
          if (session) {
            ...
            const updatedSessionData = updatedSession?.hasClientSessionData()
              ? updatedSession.getClientSessionData()
              : updatedSession?.getServerSessionData();

            if (updatedSessionData?.state === State.RECOVERED) {
              console.log(2);
              this.stopCrashDetection(sessionId);
              console.log(90);
              this.log.info(
                `${fnTag} Session ${sessionId} resolved and deleted.`,
              );
            }
          } else {
            this.log.warn(
              `${fnTag} Session not found for ID: ${sessionId}. Stopping job.`,
            );
            console.log(3);
            this.stopCrashDetection(sessionId);
          }
      ...
      this.crashDetectionJobs.set(sessionId, job);
      this.log.info(`${fnTag} Scheduled job for session ID: ${sessionId}`);
    }
  }

  public stopCrashDetection(sessionId: string) {
    const job = this.crashDetectionJobs.get(sessionId);
    if (job) {
      job.cancel();
      this.crashDetectionJobs.delete(sessionId);
      this.log.info(
        `${this.className}#stopCrashDetection() Job stopped for session ID: ${sessionId}`,
      );
    } else {
      this.log.warn(
        `${this.className}#stopCrashDetection() No job found for session ID: ${sessionId}`,
      );
    }
  }

  public stopAllCrashDetectionJobs() {
    if (this.crashDetectionJobs.size > 0) {
      this.crashDetectionJobs.forEach((job, sessionId) => {
        job.cancel();
        this.log.info(
          `${this.className}#stopAllCrashDetectionJobs() Job stopped for session ID: ${sessionId}`,
        );
      });
      this.crashDetectionJobs.clear();
      this.log.info(
        `${this.className}#stopAllCrashDetectionJobs() All jobs cleared.`,
      );
    } else {
      this.log.warn(
        `${this.className}#stopAllCrashDetectionJobs() No jobs to clear.`,
      );
    }
  }
  ...
  public async checkAndResolveCrashes(): Promise<void> {
    ...
    for (const [sessionId, session] of this.sessions.entries()) {
      ...
      if (!currentSession) {
        this.log.warn(
          `${fnTag} Updated session with ID ${sessionId} not found.`,
        );
        continue;
      }
      ...
      this.log.info(
        `${fnTag} crash state : ${State[currentSessionData.state]}`,
      );
      // remove resolved sessions
      if (currentSessionData.state === State.RECOVERED) {
      ...
      while (attempts < maxRetries) {
        ...
        if (crashStatus === CrashStatus.IN_RECOVERY) {
          ...
          if (recoverySuccess) {
            ...
            const updatedSession = this.updateSessionState(
              sessionId,
              State.RECOVERED,
            );
            this.log.info(
              `${fnTag} crash state : ${State[updatedSession.getClientSessionData().state]}`,
            );
          ...
          this.log.warn(`${fnTag} Initiating rollback!`);
          ...
          this.log.warn(`${fnTag} crash status: ${crashStatus}`);
          return;
        }
      }

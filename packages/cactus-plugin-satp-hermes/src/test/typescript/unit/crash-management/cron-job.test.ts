...
describe("CrashRecoveryManager Tests", () => {
  ...
  it("should stop cron job after session is deleted", async () => {
    jest.useFakeTimers();
    ...
    expect(crashManager["crashDetectionJobs"].has(sessionId)).toBe(true);

    crashManager["sessions"].delete(sessionId);

    jest.advanceTimersByTime(15000);
    expect(crashManager["crashDetectionJobs"].has(sessionId)).toBe(false);

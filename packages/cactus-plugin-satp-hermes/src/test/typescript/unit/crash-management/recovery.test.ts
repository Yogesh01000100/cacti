...
import { State } from "../../../../main/typescript/generated/proto/cacti/satp/v02/common/session_pb";
...
afterAll(async () => {
  if (crashManager1 || crashManager2) {
    crashManager1.stopAllCrashDetectionJobs();
    crashManager1.localRepository.destroy();
    crashManager1.remoteRepository.destroy();
    crashManager2.stopAllCrashDetectionJobs();
  ...
  if (gateway1) {
    gateway1.localRepository?.destroy();
    gateway1.remoteRepository?.destroy();
    await gateway1.shutdown();
  }

  if (gateway2) {
    gateway2.localRepository?.destroy();
    gateway2.remoteRepository?.destroy();
...
describe("Crash Recovery Testing", () => {
  it("should recover when handle recovery is called", async () => {
    ...
    const clientSession = createMockSession("20000", "3", true);
    const serverSession = createMockSession("20000", "3", false);
    ...
    const updatedSessionData = updatedSession?.getClientSessionData();

    expect(updatedSessionData?.state).toBe(State.RECOVERED);
  });
});
